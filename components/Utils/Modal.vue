...
<template>
  <dialog ref="dialog" @click="closeFromEvent" class="modal bg-transparent relative backdrop:bg-cream/75">
    <div class="max-w-[900px] mx-auto">
      <div v-if="label" class="modal-tab flex items-center gap-8 relative bg-white text-primary text-base font-bold border-3 border-b-0 w-fit py-2 px-4">
        <h2>{{ label }}</h2>
      ...
      <div class="relative -translate-y-[3px]">
        <div class="modal-content relative border-3 border-primary bg-white p-4 text-base">
          <slot />
        </div>
